---
title: "TinyGo Keeb Tour at GWC"
tags:
  - guest
organizer:
  name: "sago35 (さご)"
  avatar: "https://pbs.twimg.com/profile_images/1219630784/profile_400x400.png"
  organization: "TinyGo Keeb"
  profile: >
    日本で一番 TinyGo を書いている人。
    TinyGo Keeb のみんなではんだごてを持って全国をツアーしています。
---

TinyGo Conference 2025 in JAPAN で使用した名札である conf2025badge というハードウェアにはんだ付けを行います。
はんだ付けが終わった後は、各種デバイスを動かすソフトを書いていきます。
はんだ付け等は以下のビルドガイドに記載の流れで行います。
はんだ付けが初めての人でも、しっかりサポートするのでご安心を。

https://github.com/sago35/keyboards/blob/main/conf2025badge/build/build.md
